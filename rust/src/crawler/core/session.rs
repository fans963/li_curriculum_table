...
impl SessionManager {
    ...
    pub async fn fetch_raw(
        ...
        let mut req_builder = self.client.request(method.clone(), wrapped_url).headers(headers);
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            let cookie_val = if is_9080 {
                if !session.jsession9080.is_empty() { Some(&session.jsession9080) } else { Some(&session.jsession8080) }
            } else {
            ...
            if let Some(val) = cookie_val {
                if !val.is_empty() {
                    req_builder = req_builder.header("X-Proxy-Cookie", format!("JSESSIONID={}", val));
                }
            }
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            if let Some(logs_b64) = resp.headers().get("X-Proxy-Logs") {
                if let Ok(logs_str) = logs_b64.to_str() {
                    if let Ok(decoded_bytes) = base64::engine::general_purpose::STANDARD.decode(logs_str) {
    ...
    async fn submit_login(
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            if let Ok(new_session) = serde_json::from_value::<crate::crawler::model::ProxySession>(json["session"].clone()) {
            ...
            log::info!("[PROXY] decoded html length={}, snippet={}", text.len(), &text[..text.len().min(200)]);
            Ok(text)
        }
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            let resp = self.fetch_text(&logon_url, Method::POST, Some(body), Some(&init_url)).await?;
            Ok(resp)
        }
    }
