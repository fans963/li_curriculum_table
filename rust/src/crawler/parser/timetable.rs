...
static WEEK_RANGE_REG: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(\d{1,2})").unwrap());
...
fn merge_week_hints_into_rows(rows: Vec<CourseRow>, hints: Vec<KbtableWeekHint>) -> Vec<CourseRow> {
    // Group hints by course name once (shared read-only lookup)
    let hints_by_course: HashMap<String, Vec<&KbtableWeekHint>> = {
        let mut map: HashMap<String, Vec<&KbtableWeekHint>> = HashMap::new();
        for hint in &hints {
            map.entry(hint.course_name.clone()).or_default().push(hint);
        }
        map
    };

    // Process each row in parallel — each row only reads from the shared hint map
    rows.into_par_iter()
        .map(|mut row| {
            ...
            let course_hints = match hints_by_course.get(&row.course_name) {
                Some(h) => h,
                None => return row,
            };
...
pub(crate) fn parse_week_range(week_text: &str) -> (u32, u32) {
    let nums: Vec<u32> = WEEK_RANGE_REG
        .find_iter(week_text)
...
pub(crate) fn normalize_cell_text(html: &str) -> String {
    let text = html
        .replace("<br>", "\n")
        .replace("<br/>", "\n")
        .replace("<br />", "\n")
