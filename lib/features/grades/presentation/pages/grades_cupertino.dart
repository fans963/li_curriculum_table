  ...
  return Padding(
    ...
    child: CupertinoLiquidGlass(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Expanded(child: _statItem(context, '选中加权均分', state.selectedWeightedAverage.toStringAsFixed(2), '${state.selectedCredits.toStringAsFixed(1)} 学分')),
            Container(width: 0.5, height: 40, color: CupertinoColors.separator.resolveFrom(context).withValues(alpha: 0.4)),
            Expanded(child: _statItem(context, '总加权均分', state.weightedAverage.toStringAsFixed(2), '${state.totalCredits.toStringAsFixed(1)} 学分')),
          ],
        ),
      ),
    ),
...
Widget _statItem(BuildContext context, String label, String value, String sub) {
  return Column(
    children: [
      Text(label, style: TextStyle(fontSize: 13, color: CupertinoColors.secondaryLabel.resolveFrom(context))),
      const SizedBox(height: 4),
      Text(value, style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w700, letterSpacing: -0.5)),
      Text(sub, style: TextStyle(fontSize: 12, color: CupertinoColors.secondaryLabel.resolveFrom(context))),
    ],
  );
}
  ...
  return SliverList(
    delegate: SliverChildListDelegate([
      ...
      Padding(
        ...
        child: Row(
          children: [
            _buildCupertinoPresetButton(context, label: '必修', isSelected: _isCompulsorySelected(state), onTap: () => controller.selectCompulsory()),
            const SizedBox(width: 8),
            _buildCupertinoPresetButton(context, label: '全部', isSelected: selectedCount == allCount, onTap: () => controller.selectAll()),
            const Spacer(),
            Text('已选 $selectedCount/$allCount 门', style: TextStyle(fontSize: 13, color: CupertinoColors.secondaryLabel.resolveFrom(context))),
          ],
        ),
      ),
      ...
      ...terms.map((term) {
        ...
        double termTotalCredits = 0, termWeightedSum = 0, termCompulsoryCredits = 0, termCompulsoryWeightedSum = 0;
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: Text(term, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: CupertinoColors.systemBlue.resolveFrom(context))),
                  ),
                  const Spacer(),
                  Text('必修均分 ${termCompWavg.toStringAsFixed(2)} · 本期均分 ${termWavg.toStringAsFixed(2)}',
                      style: TextStyle(fontSize: 12, color: CupertinoColors.secondaryLabel.resolveFrom(context))),
                ],
              ),
            ),
            ...termGrades.map((grade) => _buildCupertinoGradeCard(context, grade, state)),
...
Widget _buildCupertinoGradeCard(BuildContext context, GradeEntity grade, GradeState state) {
            ...
            children: [
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: isSelected ? CupertinoColors.activeBlue : CupertinoColors.systemGrey, width: 2),
                ),
                child: isSelected ? const Icon(CupertinoIcons.check_mark, size: 14, color: CupertinoColors.white) : null,
              ),
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(grade.courseName, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 15)),
                    const SizedBox(height: 4),
                    Text('${grade.credits} 学分 · ${grade.courseAttribute}',
                        style: TextStyle(fontSize: 12, color: CupertinoColors.secondaryLabel.resolveFrom(context))),
                  ],
                ),
              ),
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Text(grade.score, style: TextStyle(color: scoreColor.resolveFrom(context), fontWeight: FontWeight.w700, fontSize: 18)),
                    Text('GRADE', style: TextStyle(fontSize: 8, fontWeight: FontWeight.bold, color: scoreColor.resolveFrom(context).withValues(alpha: 0.6))),
                  ],
                ),
              ),
            ],
...
Widget _buildCupertinoPresetButton(BuildContext context, {required String label, required bool isSelected, required VoidCallback onTap}) {
  return GestureDetector(
    ...
    child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: isSelected ? CupertinoColors.activeBlue : CupertinoColors.systemGrey5.resolveFrom(context),
        borderRadius: BorderRadius.circular(16),
      ),
      child: Text(label,
          style: TextStyle(fontSize: 13, fontWeight: isSelected ? FontWeight.w600 : FontWeight.normal, color: isSelected ? CupertinoColors.white : CupertinoColors.label.resolveFrom(context))),
    ),
  );
}
...
Widget _buildCupertinoNeedsLogin(BuildContext context) {
  return Center(
    child: Column(
      ...
      children: [
        Icon(CupertinoIcons.lock, size: 64, color: CupertinoColors.systemGrey.resolveFrom(context)),
        const SizedBox(height: 16),
        const Text('需要登录后才能查询成绩', style: TextStyle(fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        Text('请先前往「设置」页面输入账号密码', style: TextStyle(color: CupertinoColors.secondaryLabel.resolveFrom(context))),
      ],
    ),
  );
}
